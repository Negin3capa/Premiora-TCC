...
export class AvatarMigration {
   ...
   */
  private async uploadToSupabase(blob: Blob, userId: string, originalUrl: string): Promise<string | null> {
    try {
      ...
      const fileExt = validExtensions.includes(extension.toLowerCase()) ? extension : 'jpg';
  ...
  private async processUser(userData: {
    ...
    console.log(`🔄 Processando usuário ${userData.id} com avatar: ${userData.avatar_url}`);
    ...
    try {
      ...
      const newUrl = await this.uploadToSupabase(imageBlob, userData.id, userData.avatar_url);
      if (!newUrl) {
        return {
          success: false,
      ...
      console.error(`❌ Erro ao processar usuário ${userData.id}:`, errorMessage);
  ...
  async migrateOAuthAvatars(): Promise<MigrationResult> {
    ...
    try {
      ...
      console.log(`📊 Encontrados ${users.length} usuários com avatars. Verificando quais são OAuth...`);
      ...
      for (let i = 0; i < oauthUsers.length; i += batchSize) {
        ...
        console.log(`🔄 Processando lote ${Math.floor(i / batchSize) + 1}/${Math.ceil(oauthUsers.length / batchSize)} (${batch.length} usuários)`);
        ...
        batchResults.forEach((batchResult, index) => {
          ...
          if (batchResult.success) {
            result.converted++;
            console.log(`✅ [${result.processed}/${oauthUsers.length}] Usuário ${user.id} convertido com sucesso`);
          } else {
            result.failed++;
            result.errors.push({
              userId: user.id,
            ...
            console.error(`❌ [${result.processed}/${oauthUsers.length}] Usuário ${user.id} falhou: ${batchResult.error}`);
          }
        });
      ...
      console.log(`📊 Resultado: ${result.converted} convertidos, ${result.failed} falharam`);
...
export const runAvatarMigration = async (): Promise<void> => {
  try {
    ...
    console.log(result.success ? '🎉 Migração concluída com sucesso!' : '⚠️ Migração concluída com alguns erros. Verifique os logs acima.');
  } catch (error) {
