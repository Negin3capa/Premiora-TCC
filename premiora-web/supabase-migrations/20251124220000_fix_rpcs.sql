-- Migration: Fix get_feed_posts and get_post_details RPCs
-- Description: Fixes ambiguity in function overloading and column references.

-- Drop all variations of get_feed_posts to resolve ambiguity
DROP FUNCTION IF EXISTS get_feed_posts(integer,uuid,timestamp with time zone,uuid,uuid,uuid,integer,text[]);
DROP FUNCTION IF EXISTS get_feed_posts(integer,uuid,timestamp with time zone,uuid,uuid,uuid,integer);
DROP FUNCTION IF EXISTS get_feed_posts(integer,uuid,timestamp with time zone,uuid,uuid,uuid);
DROP FUNCTION IF EXISTS get_feed_posts(integer,uuid,timestamp with time zone,uuid);
DROP FUNCTION IF EXISTS get_feed_posts(integer,uuid);
DROP FUNCTION IF EXISTS get_feed_posts(integer);

-- Recreate get_feed_posts with aliased subqueries
...
-- Recreate get_post_details with aliased subqueries
